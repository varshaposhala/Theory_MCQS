...
const questions_prompts_path = "./prompts_segregation_json/" + prompts_file_name;
...
fs.writeFile(questions_response_path,
"[]", 'utf8', (err) => {
  if (err) {
...
async function withFileLock(file, callback) {
  ...
  try {
    ...
    console.error(`Error in withFileLock for ${file
        }:`, error);
    unlockFile(file);
    throw error;
    }
...
async function saveResponseToFile(file_path, response) {
  try {
    await withFileLock(file_path, async () => {
      const fileData = await readFileAsync(file_path,
            "utf8");
...
async function getQuestionPrompts() {
  try {
    const questions_prompts = await readFileAsync(questions_prompts_path,
        "utf8");
...
function getMessages(questions_prompts_json) {
  const questionObj = questions_prompts_json; // Assuming it's now a single object
  const messageObj = {
    role: "user",
...
async function start() {
  try {
    const question_prompts = await getQuestionPrompts();
    const message = getMessages(question_prompts); // Assuming single object

    // Directly call getGPTResponse since there's only one object:
    await getGPTResponse(message, question_prompts);
